...
clusterExport(cl, c("lda_mice", "lda_knn", "lda_EM.algoritem", "lda_rf",
                    "lda_complete.cases", "lda_perfect.cases", "get.data.NMAR"))
...
rez <- foreach(i = 1:nrow(zasnova), .combine = "rbind", 
            .packages = c("MASS", "mice", "missForest", 
                  ...
                  df <- get.data.NMAR(300, prop.NA = missNA.i, moc.mehanizma = moc.i, stevilo.skupin = 3)
                  df.test <- get.data.NMAR(1200,prop.NA = 0)[[1]]
                  data.NA <- df[[1]]
                  data.perfect <- df[[2]]
